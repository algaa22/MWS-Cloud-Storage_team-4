...
Если вы планируете запустить сервер на HTTPS, то перед сборкой добавьте ваши SSL-сертификаты `server.crt`, `server.csr`, `server.key`, `server.p12` в папке проекта `src/main/resources/ssl`
...
| ------ | -------------------------- | ----------------- | -------------------------------------------------- |
| `POST` | `/api/users/auth/register` | Регистрация       | `X-Auth-Email`,<br/>`X-Auth-Password`,<br/>`X-Auth-Username` |
| `POST` | `/api/users/auth/login`    | Вход              | `X-Auth-Email`,<br/>`X-Auth-Password`                   |
...
| -------- | ---------------------------------- | -------------------- | ------------------------------------------------------- |------------------------------------------------------|
| `GET`    | `/api/files/list`                  | Список файлов        | —                                                       | `directory`,<br/>`includeDirectories`,<br/>`recursive` |
| `POST`   | `/api/files/upload?path=...`       | Загрузка файла       | `X-File-Size`,<br/>`X-File-Tags`                            | —                                                    |
| `GET`    | `/api/files?path=...`              | Скачивание файла     | —                                                       | —                                                    |
| `PUT`    | `/api/files?path=...&newPath=...`  | Изменение метаданных | `X-File-Visibility`,<br/>`X-File-Tags` (хотя бы одно из них) | —                                                    |
...
| `POST` | `/api/users/update` | Обновление данных         | `X-New-Username`,<br/>`X-Old-Password`,<br/>`X-New-Password` |

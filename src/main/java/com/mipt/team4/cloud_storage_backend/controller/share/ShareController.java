...
public class ShareController {
  ...
  public void downloadSharedFile(
      ChannelHandlerContext ctx, PublicShareRequest request, HttpHeaders headers) {
    ...
    try {
      ...
      ResponseUtils.sendError(
          ctx, HttpResponseStatus.NOT_FOUND, "Share not found: " + e.getMessage());
    }
  }
  ...
  public void validatePassword(ChannelHandlerContext ctx, ValidatePasswordRequest request) {
    try {
      ShareDownloadInfo downloadInfo =
          shareService.prepareDownload(request.shareToken(), request.password());
      ShareDownloadInfo infoWithoutData =
          ShareDownloadInfo.builder()
  ...
  public void getUserShares(ChannelHandlerContext ctx, GetUserSharesRequest request) {
    List<ShareInfoResponse> shares =
        shareService.getUserShares(request.userId()).stream()
  ...
  public void getFileShares(ChannelHandlerContext ctx, GetFileSharesRequest request) {
    List<ShareInfoResponse> shares =
        shareService.getFileShares(request.fileId(), request.userId()).stream()
            .map(share -> ShareInfoResponse.fromShare(share, baseUrl))
            .toList();
    ResponseUtils.send(ctx, shares);
  }
